    ...
    if (strncmp(l, "call ", 5) == 0) {
        ...
        if (nome && valor) {
            ...
            if(valor[0]>='0' && valor[0]<='9'){
                ...
                sprintf(f->linhas[f->linha_count++], "mov eax, %s_%s",  valor, f->nome);
                sprintf(f->linhas[f->linha_count++], "mov %s_%s, eax", nome, f->nome);
            }
            came1=1;
        }
        ...
        trim(vals);
        if(val[0]>='0' && val[0]<='9'){
            sprintf(f->linhas[f->linha_count++], "mov eax, %s", val);
        }else{
            sprintf(f->linhas[f->linha_count++], "mov eax, %s_%s", val, f->nome);
        }
        ...
        if (strstr(l, "==")) {
            if(val[0]>='0' && val[0]<='9'){
               ...
               sprintf(f->linhas[f->linha_count++], "mov eax, %s_%s", var, f->nome);
               sprintf(f->linhas[f->linha_count++], "cmp eax, %s_%s", val, f->nome);
        ...
        if(values[0]>='0' && values[0]<='9'){
             ...
             sprintf(f->linhas[f->linha_count++], "mov eax,%s_%s",values, f->nome);
             sprintf(f->linhas[f->linha_count++], "mov %s_%s,eax", vars, f->nome);
         }
        sprintf(f->linhas[f->linha_count++], "%s:", label_topo);
        if (strstr(l, "==")) {
            if(val[0]>='0' && val[0]<='9'){
...
void gravar_saida(const char* nome_saida) {
    ...
    fprintf(f, "; Código Assembly 32-bit (NASM)\n\n    section .text\n [BITS 32]\n    \norg 0x11000000\n    jmp main\n\n");
